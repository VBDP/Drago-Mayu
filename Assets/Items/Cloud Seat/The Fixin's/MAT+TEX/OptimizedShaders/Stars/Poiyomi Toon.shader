Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/d55f806c2d1f0704892c6e82866540c3"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, inout PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				switch(4.0)
				{
					default: // Basic (case 1)
					{
						...
						edgeAlpha = remapClamped(da + 0.271, da, noise) * (1 - dissolveAlpha);
						break;
					}
					...
					case 3: // Spherical
					{
						...
						if (0.0)
						{
							...
							edgeAlpha = smoothstep(0.271 + .00001, 0, -normalizedDistance);
						}
						else
						{
							dissolveAlpha = (normalizedDistance < 0) ? 1 : 0;
				...
				if (0.271 || (4.0 == 2 && 0.1 != 0))
				{
					edgeColor = _DissolveEdgeGradient.Sample(sampler_MainTex, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(0.1749504,0,1,1).rgb, 0.0), float4(0.1749504,0,1,1).a);
					#ifndef POI_SHADOW
				...
				poiFragData.emission += lerp(0, dissolveToTexture * 0.0, dissolveAlpha) + lerp(0, edgeColor.rgb * 4.28, smoothstep(0, 1 - 0.0 * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			#ifdef _CUBEMAP
			...
			void applyCubemap(inout PoiFragData poiFragData, in PoiCam poiCam, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				float4 cubeMap = texCUBElod(_CubeMap, float4(CubeMapUV, (1 - 0.092) * (1 - 0.092) * 8));
				...
				poiFragData.emission += cubeMap.rgb * 0.5 * CubeMapMask * cubeMap.a;
			}
			#endif
