Shader "Hidden/Locked/.poiyomi/Old Versions/9.1/Poiyomi Toon/3714bf167dc753640946be37c2e8bc19"
{
	Properties
	{
		[HideInInspector] shader_master_label ("<color=#E75898ff>Poiyomi 9.1.22</color>", Float) = 0
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionStrength1 = 1.0;
				applyALEmmissionStrength(poiMods, emissionStrength1, float4(0,0,0,0), 0.0, float4(0,2,0,0), 0.0, 1.0);
				applyALCenterOutEmission(poiMods, poiLight.nDotV, emissionStrength1, 0.0, 0.0, float4(0,0,0,0), 1.0, 1.0);
				float glowInTheDarkMultiplier1 = calculateGlowInTheDark(0.0, 1.0, 1.0, 0.0, 0.0, 0.0, poiLight);
				#if defined(PROP_EMISSIONMAP1) || !defined(OPTIMIZER_ENABLED)
				if (!0.0)
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[0.0], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, 0.0).rgb * poiThemeColor(poiMods, float4(1,0.799103,0.6444799,1).rgb, 0.0);
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * 5.0).rgb * lerp(1, poiFragData.baseColor, 0.0).rgb * poiThemeColor(poiMods, float4(1,0.799103,0.6444799,1).rgb, 0.0);
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, 0.0).rgb * poiThemeColor(poiMods, float4(1,0.799103,0.6444799,1).rgb, 0.0);
				#endif
				if (0.0)
				{
